...
def main():
    ...
    print("1. Downloading Wikipedia Bahasa Indonesia (20231101.id)...")
    try:
        wiki = load_dataset("wikimedia/wikipedia", "20231101.id", split="train")
        wiki_path = os.path.join(out_dir, "wikipedia_id.parquet")
        wiki.to_parquet(wiki_path)
        print(f"Saved Wikipedia to {wiki_path}")
    except Exception as e:
        print(f"Failed to download Wikipedia: {e}")

    print("\n2. Downloading VQFat Indonesian Corpus (cosmopedia_id)...")
    try:
